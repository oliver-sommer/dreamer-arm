...
(:class:`dreamer_arm.envs.arms.yam.YamArm`) under different action-selection
...
Composition root for ``configs/controller/bench.yaml``. Modes
(``mode=``):
    ...
    iid       resample a Gaussian action every ``envs.action_repeat`` control
...
``nullspace_gain`` / ``max_lead_m`` instead of the single ``envs.arm.*``
...
def _build_env(arm_cfg: Any, arm_name: str, task: str, seed: int) -> tuple[Any, Any, int]:
    ...
    Mirrors ``tests/envs/test_envs.py::_make_yam_inner``, extended to accept
...
def _arm_cfg_from_cfg(cfg: DictConfig) -> Any:
    from dreamer_arm.envs.arms import ArmConfig

    return ArmConfig(**OmegaConf.to_container(cfg.envs.arm, resolve=True))
...
def run_probe(cfg: DictConfig) -> dict[str, float]:
    ...
    result = _roll_probe(
        arm_cfg,
        str(cfg.envs.arm.name),
        str(cfg.envs.task),
        int(cfg.seed),
        int(cfg.horizon),
        int(cfg.n_dirs),
    )
    log.info("probe: task=%s horizon=%d arm_cfg=%s", cfg.envs.task, cfg.horizon, arm_cfg)
    log.info(
...
def run_servo(cfg: DictConfig) -> dict[str, float]:
    ...
    arm_cfg = _arm_cfg_from_cfg(cfg)
    arm_name = str(cfg.envs.arm.name)
    if arm_name != "yam":
        raise ValueError("mode=servo requires the YAM position-servo arm")
    inner, arm, _gid = _build_env(arm_cfg, arm_name, str(cfg.envs.task), int(cfg.seed))
...
def run_sweep(cfg: DictConfig) -> None:
    ...
    arm_name = str(cfg.envs.arm.name)
    task, seed, horizon = str(cfg.envs.task), int(cfg.seed), int(cfg.horizon)
...
def _run(cfg: DictConfig) -> DiagnosticResult | None:
    """Composition root for ``configs/controller/bench.yaml``."""
    with phase("diag"):
        if bool(cfg.sweep):
            run_sweep(cfg)
            return None
...
def run() -> object:
    ...
    return run_hydra(dispatch, config_name="controller/bench", selector=("controller", "controller"))
