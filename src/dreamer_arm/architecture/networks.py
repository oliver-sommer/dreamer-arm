...
- :class:`MLP`, :class:`MLPHead` — generic MLP backbone and a distributional
  ...
  :mod:`dreamer_arm.architecture.distributions`).
- :class:`Projector`, :class:`ReturnEMA` — small helpers used by the
  R2-Dreamer Barlow-Twins head and λ-return normalisation respectively.
...
class MultiEncoder(nn.Module):
    ...
    def __init__(self, config: Any, shapes: dict[str, tuple[int, ...]]) -> None:
        ...
        shapes = {
            ...
            if k not in _EXCLUDED_OBS_KEYS and not k.startswith("log_")
        }
        self.cnn_shapes = {
            k: v for k, v in shapes.items() if len(v) == 3 and re.match(config.cnn_keys, k)
        }
        self.mlp_shapes = {
            k: v for k, v in shapes.items() if len(v) in (1, 2) and re.match(config.mlp_keys, k)
        }
...
class MultiDecoder(nn.Module):
    ...
    def __init__(
        self, config: Any, deter: int, flat_stoch: int, shapes: dict[str, tuple[int, ...]]
    ) -> None:
        super().__init__()
    ...
    def forward(self, stoch: torch.Tensor, deter: torch.Tensor) -> dict[str, Any]:
        ...
        if self._has_mlp:
            split = [v[0] for v in self.mlp_shapes.values()]
